...
| # | Challenge | Diff | Lang | ⏱️ | Why this language / what it teaches | ✅ |
|---|-----------|------|------|----|-------------------------------------|----|
| 23 | **DNS Resolver** | 🔵 | 🟦 Go | M | UDP sockets + hand-encoding/decoding the DNS wire format; recursive resolution from the root. | [ ] |
| 24 | **DNS Forwarder** | 🔵 | 🟦 Go | M | Becomes a *server*: listen on UDP, forward queries, cache responses with TTL. Depends on #23. | [ ] |
| 25 | **NTP Client** | 🔵 | 🟦 Go | S | A tight 48-byte UDP packet; struct packing and clock-offset math. | [ ] |
| 26 | **Traceroute** | 🔵 | 🟦 Go | M | Raw sockets, ICMP, and incrementing TTL to discover the path hop by hop. | [ ] |
| 27 | **Port Scanner** | 🔵 | 🟦 Go | M | TCP connect scanning at scale; goroutines + worker pools + timeouts. | [ ] |
| 28 | **Netcat** | 🔵→🟠 | 🟦 Go | M | Bidirectional TCP/UDP relay between sockets and stdin/stdout — the networking "Swiss-army knife". | [ ] |
...
| # | Challenge | Diff | Lang | ⏱️ | Why this language / what it teaches | ✅ |
|---|-----------|------|------|----|-------------------------------------|----|
| 30 | **Web Server** | 🟠 | 🟦 Go | L | HTTP/1.1 from scratch: request parsing, routing, static files, keep-alive. The server counterpart to `curl`. | [ ] |
...
| 33 | **Memcached Server** | 🟠 | 🟦 Go | L | Text/binary protocol + LRU eviction & slab-allocation concepts; instructive contrast to Redis. | [ ] |
| 34 | **NATS Message Broker** | 🟠 | 🟦 Go | L | Pub/sub, subject routing, client lifecycle, and delivery semantics. | [ ] |
| 35 | **Rate Limiter** | 🔵→🟠 | 🟦 Go | M | Token-bucket & sliding-window algorithms as reusable middleware; distributed considerations. | [ ] |
